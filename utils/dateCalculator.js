...
const dateFormatCalc = async (correlationId, getCountry) => {
  ...
  if (!country_code) {
    ...
    const countryDateS = countryDateF.split('-');

    let countrySendDate;

    if (
      countryHour[0] >= process.env.DATE_RSTART &&
      countryHour[0] < process.env.DATE_REND
    ) {
      countrySendDate = countryNow;
    } else {
      countrySendDate = moment.tz(
        countryDateS[0] +'-'+ countryDateS[1] +'-'+ (parseInt(countryDateS[2])+1) +' 07:' + countryHour[1] + ':' + countryHour[2],
        'YYYY-MM-DD HH:mm:ss',
        newArray[0].timezone
      );
    }
